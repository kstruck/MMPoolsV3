...
1. **Sentry account/DSN** — does a Sentry project exist? Provide the DSN (→ `VITE_SENTRY_DSN`), and confirm plan tier (Session Replay quota). No DSN = can scaffold masked/dev-only but can't enable sampling.

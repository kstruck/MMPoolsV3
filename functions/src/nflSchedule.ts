...
async function resolveScoreboardUrl(
  ...
  seasonType: 1 | 2 | 3,
    ...
    // fetch (Cloud Functions, Node `fetch`) with HTTP 403 (Akamai bot rule keyed
    // on the client fingerprint; browsers were unaffected), and the score sync was
    // dark for a full game day. Do not "normalise" this back to `site.api`.
    let url = `https://site.web.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard?week=${week}&season=${season}&seasontype=${seasonType}`;
    ...
    try {
      ...
      const calendarUrl = `https://site.web.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard?season=${season}`;
      ...
      if (calendarResp.ok) {
        ...
        if (segment && segment.entries) {
          ...
          if (entry && entry.startDate && entry.endDate) {
            ...
            url = `https://site.web.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard?dates=${dateQuery}`;

...
**What CI runs** (`.github/workflows/ci.yml` runs on EVERY pull request and on pushes to `main`/`master`; `security-scan.yml` runs only on pull requests that TARGET `main`/`master` and on pushes to them — so `secrets-scan`/`security-audit` never appear on a PR into a feature branch. Required-check list read from ruleset `11714546` on 2026-09-03): REQUIRED — `build-and-test` (`npm ci` root + functions → `npm run build:static` → functions `npm run typecheck` → root `npm test` → functions `npm test`), `emulator-tests` (functions `npm run test:emulator` AND `npm run test:rules` — the emulator suite and the Firestore rules suite BOTH run in CI), `nginx-validate` (`nginx -t` in docker), `lint` (required since 2026-09-03, `continue-on-error` removed; 0 errors / ~1870 warnings baseline, warnings do not fail it), `secrets-scan`, `security-audit` (`npm audit --audit-level=high`). RUNS BUT NOT REQUIRED — `e2e-playwright` (Playwright against real emulators; a red run cannot block a merge). **NOT in CI:** any manual UI verification. Git hooks: exactly one, `.husky/pre-commit` — `python scripts/scan_secrets.py` (blocking under `sh -e`) followed by an advisory warning when more than 10 paths are staged (echo only; pinned by `tests/pre-commit-file-count.test.ts`). No local test gate.

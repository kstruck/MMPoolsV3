...
2. **The Gemini-leak claim — REVERSED 2026-08-23.** `CODE_REVIEW_REPORT.md:183` and `AUDIT-REPORT.md:147,278` were RIGHT: `git show 3340fff0^:.env | grep -c VITE_API_KEY` (count-only — never reprint the value) in the public repo shows `VITE_API_KEY` (a Gemini key), exposed since 2025-12-13. The 2026-07-06 owner denial was the false claim, and it propagated into nine skill files (all corrected 2026-08-23). Lesson upgraded: a "ground truth" statement that contradicts a measurable command is a hypothesis — run the command. Rotation CLOSED 2026-08-24 (Kevin ruling, evidence-verified): the leaked value returns API_KEY_INVALID when tested live, and .env history contains no other private key — the live key ("New MarchMeleePoolsAPI2", Jan 2026) never touched git. Kevin had already rotated; no further action.. (The Stripe TEST-secret item in `functions/.env` is separate and real.)
...
Additional context that must never be contradicted when editing docs (owner ground truth, 2026-07-06): Stripe is commissioner hosting fees ONLY — the platform never touches participant entry fees (P2P honor system); NFL pools have never operated a live season (2026 is the first; `scoreNFLWeek` is a manual per-pool/per-week callable; `lockNFLSpreadsJob` exists in code but was never exported/deployed — see **mmp-nfl-season-campaign**); App Check ENFORCEMENT: that 2026-07-06 line is SUPERSEDED and must NOT be treated as uncontradictable - as of 2026-07-30 App Check is enforced NOWHERE (98 validated() callables on monitor, zero enforce, 26 bare onCall sites with no option) and setting VITE_RECAPTCHA_SITE_KEY preceded a production outage; HANDOFF STOP POINT box is authoritative; Phase 3.1 functions + tightened rules are deployed and the searchName backfill has run.
...
Drift-prone facts date-stamped in this file: deploy state (Phase 3.1 live, ~~App Check enforced~~ **— that one DRIFTED and is corrected 2026-07-30: App Check is enforced NOWHERE, and setting `VITE_RECAPTCHA_SITE_KEY` preceded a production outage; see HANDOFF's STOP POINT box**, backfill run, Stripe TEST rotation pending), autoClosePools live-past-dry-run, wizard worktree location `D:\mmp-wizard` (a fact about Kevin's machine, not a source of record), and everything in the stale-docs table (rows disappear as fixes land — prune them when they do).

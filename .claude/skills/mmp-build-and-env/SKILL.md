...
| Var | Read at | Notes |
|---|---|---|
| `VITE_RECAPTCHA_SITE_KEY` | src/firebase.ts:24 | App Check (ReCaptcha Enterprise). If absent, App Check is not initialized; non-dev builds log a security warning (src/firebase.ts:30-32). In dev mode the App Check debug-token flag is set (src/firebase.ts:20-23). App Check is ENFORCED in the prod console as of 2026-07-06; exact local-dev impact of enforcement when this key is missing is UNVERIFIED — verify against the Firebase console App Check page if callables start failing with App Check errors. |
...
| Console warn `App Check is NOT active` in a prod build | `VITE_RECAPTCHA_SITE_KEY` absent at build time | Supply the key (Firebase console → App Check) for prod-like builds; App Check is enforced in prod as of 2026-07-06 |

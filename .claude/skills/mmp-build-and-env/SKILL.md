...
| Var | Read at | Notes |
|---|---|---|
| `VITE_RECAPTCHA_SITE_KEY` | src/firebase.ts:24 | ⛔ **DO NOT SET THIS IN PROD.** Setting it in Coolify took production down on 2026-07-30 (blank page, spinner forever) and was rolled back by deleting it. Absent is the CORRECT state; the `App Check is NOT active` warning it produces is the SAFE state. See §7's troubleshooting row and HANDOFF's STOP POINT box for the mechanism and the three faults that must be fixed first. In dev mode the App Check debug-token flag is set (src/firebase.ts:20-23), which is unaffected. The earlier claim here that "App Check is ENFORCED in the prod console as of 2026-07-06" is **superseded and UNVERIFIED** — every callable is `monitor` (98/98, zero `enforce`) and the web app was never registered for App Check. |
| `GEMINI_API_KEY` | `.agent/skills/front-end-skill` (image-generation agent skill) | NOT read by the app. Note: the live `.env` contains a stale comment claiming this key was committed/leaked — per owner interview 2026-07-06 the Gemini key was NOT leaked; treat that comment as wrong. |
...
| Console warn `App Check is NOT active` in a prod build | `VITE_RECAPTCHA_SITE_KEY` absent at build time — **deliberately** | ⛔ **LEAVE IT. This warning is the safe state, not a defect.** The previous advice in this row ("supply the key") is what took prod down on 2026-07-30: the key makes `src/firebase.ts:25` initialize `ReCaptchaEnterpriseProvider`, whose script CSP blocks, so the App Check token never resolves and the Firestore SDK — which waits on it before its first request — times out after ~10s and goes offline. Blank page. Server-side `monitor` does not help; the failure is client-side before any request leaves. Three faults must ALL be fixed first (CSP hosts, Enterprise-vs-v3 key, app never registered) — see HANDOFF's STOP POINT box. |

...
| S3 | `firebase deploy --only functions` fails with `TS2307: Cannot find module 'stripe'` (or `'firebase-functions-test'`) | `functions/node_modules` not installed — the predeploy hook runs `npm --prefix functions run build` (`firebase.json`) which needs functions' own deps (root install does NOT cover them) | `Test-Path functions/node_modules/stripe` | Run `npm --prefix functions install` from repo root, then redeploy. This is deploy-ritual rule (b) in mmp-change-control |
...
- **T5 TS2307 stripe/fft.** Recurring deploy-time failure: the functions predeploy
  ...
  repeated hits. Cure is always `npm --prefix functions install` first.

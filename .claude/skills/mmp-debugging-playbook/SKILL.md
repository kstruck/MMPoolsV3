...
- **App Check** — Firebase attestation (ReCaptcha Enterprise). **Enforced NOWHERE**: 98 `validated()` callables declare `appCheck: "monitor"` and zero declare `enforce`, plus 26 bare `onCall` sites with no App Check option at all (`lib/validated.ts:94-97`). The client never initializes it in prod either, because `VITE_RECAPTCHA_SITE_KEY` is deliberately absent — ⛔ setting it took prod down on 2026-07-30. The 2026-07-06 "ENFORCED in the prod console" attestation is superseded and UNVERIFIED.
...
| S7 | Requests rejected before code runs; attestation/App Check errors; works in DEV but not a prod-like build | **App Check is enforced nowhere** — 98 `validated()` callables are `monitor`, zero `enforce`, plus 26 bare `onCall` sites with no App Check option, zero `enforce` (`lib/validated.ts:94-97`) — so App Check is almost certainly NOT your cause; look elsewhere before spending time here. The client only initializes it when `VITE_RECAPTCHA_SITE_KEY` is set (`src/firebase.ts:24-32`); DEV sets a debug-token flag (`:20-23`) | Console warning `App Check is NOT active` in a non-DEV build = missing site key, **and that is the intended prod state**. In e2e/emulator it never applies (`.env.e2e` has no site key; emulators don't enforce) | ⛔ **NOT "set `VITE_RECAPTCHA_SITE_KEY`".** Setting it in Coolify on 2026-07-30 was followed by a blank-page outage; deleting it restored service. ⚠️ **Why is an OPEN QUESTION** — the CSP-blocks-the-script story is unproven and `Dockerfile:15-27` declares no `ARG` for that key, so it has no known path into the bundle. **Four** faults block re-enabling; see HANDOFF's STOP POINT box and row S7b. Note `logClientError` still has `enforceAppCheck: false` + TODO (`functions/src/logClientError.ts:33-35`) |
| S7b | The whole site renders nothing — permanent spinner, no data, no error — right after a Coolify rebuild | This is the 2026-07-30 outage. **What is known:** setting `VITE_RECAPTCHA_SITE_KEY` in Coolify preceded it and deleting it fixed it. **What is NOT known: why.** The first theory (Firestore SDK blocking on an App Check token CSP won't let resolve) is holed — `Dockerfile:15-27` declares no `ARG` for that key, so it has no known path into the bundle. Treat the cause as OPEN | Read the Coolify environment for `VITE_RECAPTCHA_SITE_KEY`. Then check the browser console for a CSP refusal of `https://www.google.com/recaptcha/enterprise.js` — **if it is absent, the App Check theory is wrong for your incident too** and you should compare the two Coolify deploy logs and diff the served bundle instead | **Delete the variable and redeploy** — that restored service. Then actually diagnose it; an unexplained way to kill prod is worse than an understood one |
...
| Prod-console facts (autoClose live, Coolify manual deploy) | NOT verifiable from the repo — confirm with Kevin or the Firebase/Coolify consoles; treat the 2026-07-06 statements here as the last known state. **"App Check enforced" is no longer among them** — it is superseded (2026-07-30) and the code is checkable: `Get-ChildItem functions\src -Recurse -Filter *.ts \| Select-String -Pattern 'appCheck: .enforce.'` returns nothing |

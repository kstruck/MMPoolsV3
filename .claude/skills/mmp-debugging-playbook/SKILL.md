...
- **App Check** — Firebase attestation (ReCaptcha Enterprise). **Enforced NOWHERE**: 98/98 callables declare `appCheck: "monitor"`, zero `enforce` (`lib/validated.ts:94-97`). The client never initializes it in prod either, because `VITE_RECAPTCHA_SITE_KEY` is deliberately absent — ⛔ setting it took prod down on 2026-07-30. The 2026-07-06 "ENFORCED in the prod console" attestation is superseded and UNVERIFIED.
...
| S7 | Requests rejected before code runs; attestation/App Check errors; works in DEV but not a prod-like build | **App Check is enforced nowhere** — 98/98 callables are `monitor`, zero `enforce` (`lib/validated.ts:94-97`) — so App Check is almost certainly NOT your cause; look elsewhere before spending time here. The client only initializes it when `VITE_RECAPTCHA_SITE_KEY` is set (`src/firebase.ts:24-32`); DEV sets a debug-token flag (`:20-23`) | Console warning `App Check is NOT active` in a non-DEV build = missing site key, **and that is the intended prod state**. In e2e/emulator it never applies (`.env.e2e` has no site key; emulators don't enforce) | ⛔ **NOT "set `VITE_RECAPTCHA_SITE_KEY`".** Setting it in Coolify took prod down on 2026-07-30 — CSP blocks the reCAPTCHA script, the token never resolves, the Firestore SDK waits on it and goes offline, blank page. Three faults must be fixed first; see HANDOFF's STOP POINT box. Note `logClientError` still has `enforceAppCheck: false` + TODO (`functions/src/logClientError.ts:33-35`) |
| S7b | The whole site renders nothing — permanent spinner, no data, no error — right after a Coolify rebuild | `VITE_RECAPTCHA_SITE_KEY` was set in the Coolify environment. This is the 2026-07-30 outage, reproduced. The Firestore SDK blocks its first request on an App Check token that CSP prevents from ever resolving | Read the Coolify environment for `VITE_RECAPTCHA_SITE_KEY`; in the browser console look for a CSP refusal of `https://www.google.com/recaptcha/enterprise.js` followed by Firestore going offline after ~10s | **Delete the variable and redeploy.** That is the whole rollback |

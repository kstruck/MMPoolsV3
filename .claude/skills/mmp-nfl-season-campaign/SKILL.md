...
3. **Scoring before the last game of the week is FINAL (SUPER_ADMIN bypass).** The ACTIVE_GAMES guard blocks owners but not SUPER_ADMIN. An early score is now a *provisional* pass — `scoreNFLWeek` derives `provisional: !isWeekComplete(...)` — but ⚠️ **provisional does NOT mean inert, and reading it that way is how someone strikes a member by accident.** What it actually does (`scoreWeekPass`, nflPools.ts):
   - **Withholds only the FINALIZATION artifacts**: the `scoredWeeks` marker, `maybeFinalizeNFLPool`, the weekly recap and the `SCORE_FINALIZED` audit.
   - **Grades only "revealed" games** — `gradableGames = provisional ? games.filter(revealed) : games`.
   - **STILL WRITES PER-ENTRY UPDATES.** Pick'em always; Survivor and Margin once `weekLockPassed`.
   - **STILL APPLIES NO-PICK PENALTIES after the weekly lock** — a Survivor strike, a Margin `-14`. `weeklyPickReady` (`:1062`) returns `true` on a provisional pass once the deadline has passed, because **the penalty is due at the WEEKLY LOCK, not at week completion.** Before the lock it holds both the penalty and made picks (a CANCELLED game is terminal but its pick window may still be open).
   - Writes `standings/current`, and returns `Week N scored provisionally — live standings updated.`

   So the surviving hazard of scoring early is narrower than the original entry claimed but real: **after the weekly deadline, a provisional pass penalises non-submitters exactly as a final one would.** What is genuinely obsolete is the rest of the original text — "strikes survivors for non-submission while PER_GAME-lock players could still legally pick a later game" and "margin entries with pending games score `null→0`" — both of which the reveal gate now prevents.
   **RETIRED, and it was the dangerous half of this entry:** *"If you must re-run after an early mistake, remember §4.1: survivor re-runs double-strike."* They do not, and have not since `248f61a` — **re-running after an early score is exactly the right recovery and this sentence told operators not to.** Waiting for FINAL before scoring is still correct; fearing the rescore is not.

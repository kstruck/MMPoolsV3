...
3. **Scoring before the last game of the week is FINAL (SUPER_ADMIN bypass).** The ACTIVE_GAMES guard blocks owners but not SUPER_ADMIN. ⚠️ **The consequences below are LARGELY OBSOLETE and are kept only to describe the pre-2026-07 behaviour** — an early score is now a *provisional* pass: `scoreNFLWeek` derives `provisional: !isWeekComplete(...)` and a provisional pass withholds Survivor strikes, Margin `-14`s, the `scoredWeeks` marker, finalization and the recap, writing only `standings/current`. It returns `Week N scored provisionally — live standings updated.` The old consequences were: strikes survivors for non-submission while PER_GAME-lock players could still legally pick a later game; breaks the rebuy window the README promises; margin entries with pending games score `null→0`.

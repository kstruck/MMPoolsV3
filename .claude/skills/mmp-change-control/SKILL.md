...
|---|---|---|
| **Ordinary change** | Anything that touches none of the Rule-3 triggers below — **any file count**, from a one-line typo to a 14-file refactor | Own branch off `origin/main`, all five gates green, `codex exec review --base origin/main` (**5 rounds max** — the cap is the stopping rule, CLAUDE.md §2c; at 5 with findings still open, stop and name them in the PR body), PR through CI. **No plan doc.** Own worktree if another session may be active (Rule 4). |
...
| **Firebase console toggles** | App Check enforcement, function rollback via console, Cloud Scheduler job approval — console access | App Check ENFORCED in console (done) |

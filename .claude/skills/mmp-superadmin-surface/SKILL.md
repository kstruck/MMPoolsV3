...
**Prod deploy state (owner ground truth, as of 2026-07-06):** Phase 3.1 functions (`onUserCreated`, `syncAllUsers`, `searchUsersByEmail`) + `adminHealth` functions deployed; tightened firestore.rules deployed (functions-before-rules order respected); `searchName` backfill (Force Sync) run; ~~App Check ENFORCED in the Firebase console~~ **— SUPERSEDED and UNVERIFIED as of 2026-07-30: App Check is enforced NOWHERE (98 `validated()` callables on `monitor`, zero `enforce`, plus 26 bare `onCall` sites with no option), the client never initializes it in prod, and setting `VITE_RECAPTCHA_SITE_KEY` preceded a production outage. HANDOFF's STOP POINT box**; `autoClosePools` LIVE past dry-run. **Still pending: Stripe TEST secret rotation** (plaintext test key in `functions/.env` — delete + rotate in Stripe dashboard; Kevin-only action). CORRECTED 2026-08-23: a Gemini key (`VITE_API_KEY`) WAS leaked in public git history (`git show 3340fff0^:.env | grep -c VITE_API_KEY` (count-only — never reprint the value), exposed since 2025-12-13); Rotation CLOSED 2026-08-24 (Kevin ruling, evidence-verified): the leaked value returns API_KEY_INVALID when tested live, and .env history contains no other private key — the live key ("New MarchMeleePoolsAPI2", Jan 2026) never touched git. Kevin had already rotated; no further action.. Note: `PHASE0-DEPLOY-CHECKLIST.md` says "Coolify auto-builds main on push" — the owner states prod www deploys are a **manual trigger in the Coolify dashboard**; treat push-to-main as NOT deploying the frontend (see mmp-deploy-and-operate).
...
- Unaudited Export Emails; ~~`enforceAppCheck` re-enable on logClientError~~ **— WITHDRAWN 2026-07-30, do not do this: its premise (App Check enforced in the console) was never true, and acting on that premise from the client side preceded a production outage. See HANDOFF's STOP POINT box**; T8 playoff-entries 1MB doc migration (never built, three audits running)

...
4. **Mid-season setting flips are a real retroactivity hazard — for BOTH fields** (codex r1 #4): `computeSurvivorWeekUpdate` recomputes past weeks with *current* settings, so flipping `tieCountsAs` after a tied week rewrites history on rescore, and changing `maxTeamUses` can add/remove auto-survive exemptions with the same effect. UI-only gating is insufficient — the callable stays reachable and super-admin bypasses the UI. **Plan of record (codex r2 #1, promoted from recommendation): server-side rejection is a required Phase 3 deliverable**, implemented in the `updatePoolSettings` write path inside a transaction that reads the pool doc (the current callable writes after a non-transactional read — `poolOps.ts:400-466` — and `flattenSettingsPatch` sees only `set`+`poolType`, so the check lives in the callable, not the flattener). "Scored" := any **true** marker in `publishedWeeks` OR `legacyPublishedWeeks(pool).length > 0` — the helper (`functions/src/lib/publishedWeeks.ts:34,50`) already folds in true `scoredWeeks` and the legacy `scoredThroughWeek` high-water mark; `false` markers mean UNSCORED and must not trip the gate (codex r5 #1). `scoredWeeks` is withheld on provisional passes (`nflPools.ts:1455-1464`), so `publishedWeeks` covers the provisional window; lifecycle status is NOT a proxy (a pool can hold scored survivor outcomes while lifecycle-open — `editability.ts:28-44`). Reject only a **value change** to either field, and evaluate each field **only when that property is present in the incoming settings patch** (codex r5 #2 — `flattenSettingsPatch` applies present keys only, so a partial `{maxStrikes: 2}` save on a scored `tieCountsAs:'WIN'` pool must not be rejected over an omitted field). When present, compare **effective normalized values** — `current ?? default` vs `incoming ?? default` — so a scored legacy pool saving UI defaults is not rejected over `undefined → 'LOSS'` (codex r3 #2). The write itself routes through the **`retryWhileScoring` transaction pattern** (codex r7 #1): re-read the pool in-transaction, reject while a scoring lease is live — `updatePoolSettings` today serializes only lock-affecting fields (`poolUpdate.ts:98`), and the manual scorer re-reads post-lease (`nflPools.ts:986`), so without this a concurrent edit lands between re-read and publication. **Limit-reduction invariant** (codex r7 #2): reducing `maxTeamUses` to a positive value is rejected when any entry's canonical `countTeamUses(picks)` exceeds it — entries read inside the same transaction. **Bypass closure** (codex r8 #1, mechanism corrected r9 #1): `firestore.rules` gains a deny for ANY client write — super-admin included — where the **nested settings diff** `request.resource.data.get('settings', {}).diff(resource.data.get('settings', {})).affectedKeys()` contains bare `tieCountsAs` or `maxTeamUses`, applied OUTSIDE the super-admin disjunction. Root-level `affectedKeys()` cannot see nested fields (documented at `firestore.rules:138` / `poolUpdate.ts:127`) — the round-8 wording would never have fired. Privileged edits go through the gated callable. Emulator coverage: dotted update, wholesale `settings` replacement, field deletion, unrelated SA settings update still allowed. Scoped to the two fields on purpose: a blanket SA lockout of NFL settings would change the SA repair surface beyond this plan's blast radius. Admin-SDK/console writes bypass rules by nature — documented as carrying the same invariant. Deploy ordering: functions BEFORE rules (Rule 2). Tests: interleaving vs `scoreNFLWeekInternal`, provisional-score, legacy-defaults-save, partial-unrelated-save on a scored non-default pool, `0→1`/`2→1` reduction rejection, increases permitted, emulator rules test that a SA direct write of either field is denied. Kevin may veto in favor of per-week snapshotting (question 1).

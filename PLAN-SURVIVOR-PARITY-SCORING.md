...
- **`countTeamUses(picks, excludeWeek)` contract** (codex r2 #3, grammar tightened r3 #3): iterate `Object.entries(picks)`; a key COUNTS only if it matches `/^\d+$/` AND `Number(key)` is in 1–23 (the `submitNFLPicksSchema` week range); exclude by NUMERIC equality with `excludeWeek`. Expected results pinned by test: `"1"`, numeric access, `"01"` → week 1 (excluded when `excludeWeek` is 1); `"1.5"`, `"2junk"`, `" 1"`, `"1e0"` → skipped.

...
| `functions/src/lib/poolDues.ts:67,86-87,107` | `poolRef.collection('private').doc(`${DUES_PREFIX}${uid}`)` (`:67`); `readPoolDues` reads `snap.data()?.paidEntries` and returns it only past an object/non-array check (`:86-87`); `writePoolDues` does `tx.set(poolDuesRef(...), { uid, poolId, paidEntries, updatedAt: now })` — **no merge**, so a removal is expressible (`:107`). (An earlier draft cited `:82`, the parameter, for the write — codex r6.) | ✅ **NEW in P2-T3**, extended in T7 to export `DUES_PREFIX`/`DUES_PREFIX_END` so the two id-range callers cannot drift. |
...
| `functions/src/nflEntryDelete.ts:228,229,251` | `playableEntryCount: after.playableEntryCount` — written into `memberAfter` for the liability arithmetic (`:228`, with `hasPlayableEntry: after.playableEntryCount > 0` at `:229`) and into the `memberPatch` that lands on the Member Record (`:251`). Recounted from the surviving documents, not decremented. ✅ **P2-T4** — the path that made the field reversible. (`:8,32,210` are comments; citing them for a claimed write was inaccurate — codex r6.) |

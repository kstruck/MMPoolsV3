...
| 9 — visibility + league-avg line (gap closure) | ✅ 2026-07-19 | Post-ship audit found: no UI entry points to /profile/:uid, empty publicProfiles pre-season, and the reference design's "vs average" chart line never built. Added: standings names + header "My Stats" link to profiles; self-heal recomputeMyProfile on own empty profile; publicProfiles/_siteAverages daily aggregate (siteAveragesJob + refreshSiteAverages) feeding a dashed League Avg chart line; TEMPORARY /dev/profile-demo marketing page (dummy data — delete DevProfileDemo.tsx + route after use). |

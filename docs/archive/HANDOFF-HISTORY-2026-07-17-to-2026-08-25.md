# HANDOFF history — 2026-07-17 through 2026-08-25 (archived 2026-09-08)

Moved here **verbatim** from the top-level `HANDOFF.md` on 2026-09-08 so the
live handoff stays short enough to read at every session start. Newest box
first, as it was in the original.

**Nothing here is current state.** `HANDOFF.md` is the live-state carrier and
`CONTEXT.md` the glossary. Every deploy-state line below was already
superseded by a later box before it was moved; the tagged live SHA lives in
`HANDOFF.md` only. Open a box here to recover the reasoning behind a decision
that a live doc cites, not to learn what is deployed.

Link destinations were rewritten for this location (`../../` for files that
live at the repo root, bare names for files that also live in this directory);
nothing else in the moved text was changed.

## Box index (search the heading text; newest first, as in the original)

Other docs — `PICKUP-PRESEASON-PILOT.md` in particular — cite "HANDOFF's STOP POINT box" or
"DEPLOY STATE" by date. Every `##` box in this file is listed here so a cited box can be found
by its date without reading the file top to bottom. Labels are truncated; search the date.

- ✅ 2026-08-25 — LAUNCH DAY. HISTORY, and its deploy facts are SUPERSEDED by the box above. Eight PRs merged …
- 📌 2026-08-25 — RULESET GOVERNANCE: ANY REVIEW REQUIREMENT ON `main` MUST KEEP "Repository admin" IN THE …
- 🟢 2026-08-25 (latest) — OVERNIGHT AUDIT REMEDIATION, SESSION 2: SEVENTEEN PRs MERGED (#550, #564–#579). THE …
- 🟢 2026-08-24 (superseded by the 2026-08-25 box above) — OVERNIGHT AUDIT REMEDIATION: NINE PRs MERGED …
- 🟢 2026-08-25 …
- 🟢 2026-08-25 (earlier) — FOUR PRs MERGED: T10 (THE LAUNCH BLOCKER), THE POOL FEED + PIN, AND BOTH HALVES OF …
- 🟢 2026-08-24 (latest) — PLAN-WIZARD-BUYFLOW-FIXES: ELEVEN PRs MERGED (9 of 10 TICKETS). THE LAUNCH FLIP IS …
- 🟢 2026-08-22 (latest) — §2c IS NOW CLOSED ON THE SIX MERGED PRs. THE HELP `?` IS VISIBLE (#514, MERGED). …
- 🟢 2026-08-22 (later) — THE ENVIRONMENT FIX WORKS. PR #513's GATES ARE ALL RUN — REVIEW LOG + SWEEPS LANDED. …
...
- 🟢 2026-08-21 (later) — #495 IS LIVE AND CONFIRMED. TWO OF THE SIX ARE STILL OPEN, ONE NEEDS A RULING FROM …
...
- 🛑 2026-08-18 — PLAN-PAYMENT-LEDGER T2 IS OPEN (Coolify-only); the whole ledger stack through T1 is MERGED …
- 🛑 2026-08-17 (late 08-16) — LEDGER UX FIX IN PR #460 (Coolify-only); the six-PR ledger stack #451→#456 + …
- 🛑 2026-08-17 (overnight 08-16) — THE PAYMENT LEDGER: SIX STACKED PRs OPEN (#451→#456), NOTHING MERGED OR …
- 🛑 2026-08-17 (overnight) — PLAN-MULTI-ENTRY T2 (submit path + dues) IN A PR; T1 (#449) + co-commissioners …
...
- ✅ DEPLOY STATE 2026-08-12 — HISTORY. functions + rules were at <!-- deploy-state:ignore --> `main` @ …
- ⚠️ SUPERSEDED — 🚀 STOP POINT 2026-08-11 (evening) — everything merged and deployed; the spreads mystery is …
- ⚠️ SUPERSEDED — STOP POINT 2026-08-10 (overnight) — LAUNCH WEEK: two gated PRs await Kevin; invites unblocked …
- ✅ DEPLOY STATE <!-- hof-date:ignore --> 2026-08-09 — functions + rules + frontend all at `c7bdcf5` * …
- ⚠️ SUPERSEDED — STOP POINT <!-- hof-date:ignore --> 2026-08-08 (late) — FUNCTIONS DEPLOY + COOLIFY REBUILD …
- ⚠️ SUPERSEDED — STOP POINT <!-- hof-date:ignore --> 2026-08-08 — BACKEND QUEUE EMPTY, DATA REPAIR DONE, HOF …
- ⚠️ SUPERSEDED — STOP POINT <!-- hof-date:ignore --> 2026-08-07 — FUNCTIONS DEPLOY OWED (#392 + #384), THEN A …
- 🏈 STOP POINT 2026-08-06 — REBUILD DONE AND VERIFIED PER PR; ALL FOUR QUEUES EMPTY; functions unchanged at …
- ✅ STOP POINT 2026-08-05 (night) — THE BILLING JOB RAN CLEAN, first success ever; #374's rebuild RAN and is …
...
- Prior state: 11 SWEEP-LATER callables remain (10 actionable + createBracketPool deferred) — batches 1-4 …
...
>    ([PLAN-AUDIT-AUTH-HARDENING-SWEEPS.md](../plans/PLAN-AUDIT-AUTH-HARDENING-SWEEPS.md)).
...
>
> 🆘 **Site down? Start at [RUNBOOK-SITE-DOWN.md](../runbooks/RUNBOOK-SITE-DOWN.md)** — one page: uptime alert → bundle-hash curl → debugging-playbook S7b → Coolify Rollback (§2b) → `/readiness` → `functions:log` → functions rollback (§1d).

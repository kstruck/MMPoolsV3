...
export const PayoutSummaryCard: React.FC<PayoutSummaryCardProps> = ({ gameState, winners }) => {
                    ...
                    <div className="space-y-3">
                        ...
                        {gameState.charity?.enabled && (
                            <div className="flex justify-between items-center text-sm border-b border-slate-800 pb-2 text-rose-300">
                                ...
                                <span className="font-mono font-bold">
                                    -${(Math.floor((totalPot * (gameState.charity.percentage / 100)))).toLocaleString()}
                                </span>
                            </div>
                        )}
                        ...
                        <div className="flex justify-between items-center text-sm border-b border-slate-700 pb-2 mb-2">
                            ...
                            <span className="text-emerald-400 font-mono font-bold text-lg">
                                ${(Math.floor((totalPot * (1 - (gameState.charity?.enabled ? gameState.charity.percentage / 100 : 0))))).toLocaleString()}
                            </span>
                        </div>
                    </div>

                  ...
                  <span className="flex items-center gap-1.5 ml-1 inline-flex">
                    {(castPool.contactMethod === 'email' || castPool.contactMethod === 'both' || !castPool.contactMethod) && pool.contactEmail && (
                      <a
                        ...
                        className="p-1 bg-navy-800/5 hover:bg-navy-800/10 text-navy-700 dark:bg-gold-400/10 dark:hover:bg-gold-400/20 dark:text-gold-400 border border-line rounded-sm transition-ui duration-150 fine:hover:scale-105 flex items-center justify-center cursor-pointer"
        ...
        >
          {TAB_STRIP.filter(({ tab }) => tabOffered[tab]).map(({ tab, label }) => (
            <button
              ...
              className={`py-3 px-4 md:px-6 font-display font-bold uppercase text-[13px] tracking-[0.08em] transition-ui duration-150 border-b-2 ${
                activeTab === tab

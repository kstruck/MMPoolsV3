...
const App: React.FC = () => {
  ...
  const route = useMemo(() => {
    ...
    if (hash.startsWith('#super-bowl-squares-odds')) return { view: 'super-bowl-squares-odds', id: null };
  ...
  if (route.view === 'super-bowl-squares-odds') {
    return (
      <>
        <Header user={user} onOpenAuth={() => setShowAuthModal(true)} onLogout={authService.logout} onCreatePool={handleCreatePool} />
                      ...
                      {(squaresPool.paymentHandles?.venmo || squaresPool.paymentHandles?.zelle) ? (
                        <div>
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            {squaresPool.paymentHandles?.zelle && (
                              <div className="bg-slate-800 border border-slate-700 text-white px-3 py-2 rounded-lg text-sm font-bold flex items-center gap-2 justify-center w-full">
                                <span className="text-slate-400 text-xs uppercase mr-1">Zelle:</span> {squaresPool.paymentHandles.zelle}
                                <button
                                  onClick={() => {
                                    navigator.clipboard.writeText(squaresPool.paymentHandles?.zelle || '');
                                    setZelleCopied(true);

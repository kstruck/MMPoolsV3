          ...
          <div className="min-h-screen bg-slate-900 text-slate-100 font-sans">
            ...
            <main className="max-w-3xl mx-auto p-6 mt-10">
              <div className="text-center mb-12">
                <h2 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-emerald-400 mb-4">Public Grids</h2>
        ...
        return (
          <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-indigo-500 selection:text-white pb-20 relative">
            ...
            <div className="max-w-[1400px] mx-auto px-4 pt-6 flex justify-between items-center">
              ...
              <div className="text-center"><h1 className="text-3xl font-bold text-white mb-1">{currentPool.name}</h1><p className="text-slate-400 text-sm">{100 - currentPool.squares.filter(s => s.owner).length} Squares Remaining</p></div>
            ...
            <div className="max-w-[1400px] mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-black rounded-xl border border-slate-800 p-6 shadow-xl flex flex-col justify-center">
                ...
                <div><h3 className="text-slate-500 font-bold uppercase text-xs mb-1">Instructions:</h3><p className="text-slate-300 text-sm leading-relaxed">{currentPool.paymentInstructions}</p></div>
              </div>
              {/* Scoreboard */}
            ...
            <div className="max-w-[1600px] mx-auto px-4 grid grid-cols-1 xl:grid-cols-[300px_1fr_300px] gap-8 items-start mb-8">
              ...
              <div className="flex flex-col items-center">
                <div className="flex items-center gap-4 w-full">
                  ...
                  <div className="hidden md:flex flex-col items-center gap-2"><div className="w-16 h-16 bg-indigo-900/20 rounded-full flex items-center justify-center border-2 border-indigo-500 shadow-[0_0_20px_rgba(99,102,241,0.3)] bg-white p-1">{getTeamLogo(currentPool.awayTeam) ? <img src={getTeamLogo(currentPool.awayTeam)!} className="w-full h-full object-contain" /> : <span className="text-indigo-400 font-bold text-xl">{currentPool.awayTeam.substring(0, 2).toUpperCase()}</span>}</div></div>
                </div>
              </div>
              <div className="hidden xl:block">
                <div className="border border-amber-500/30 rounded-xl p-0 overflow-hidden">
                  <div className="bg-gradient-to-r from-slate-900 to-slate-800 p-4 border-b border-slate-800 flex items-center gap-2">{getTeamLogo(currentPool.homeTeam) && <img src={getTeamLogo(currentPool.homeTeam)!} className="w-8 h-8 object-contain" />}<h3 className="text-amber-400 font-medium text-sm">If the <span className="text-rose-400 font-bold">{currentPool.homeTeam}</span> score next...</h3></div>
                  <div className="bg-black/50 p-4 space-y-4">{calculateScenarioWinners(currentPool, 'home').map((pred) => (<div key={pred.points} className="flex justify-between items-center group border-b border-slate-800/50 pb-2 last:border-0 last:pb-0"><div><span className="block text-slate-300 font-bold text-sm group-hover:text-rose-400 transition-colors">+{pred.points} points</span><span className="text-[10px] text-slate-500">New digit: {pred.newDigit}</span></div><span className="text-white font-bold text-sm">{pred.owner}</span></div>))}</div>
                </div>
              </div>
            </div>
            {/* Bottom Payout Cards */}

...
### Pool
A contest (bracket, squares grid, pick'em, survivor, margin, prop bet) managed by a Commissioner or Super Admin. A Pool has one owner (`ownerId`), optionally a separate manager (`managerUid`), and — on the three NFL types only — up to three named Co-Commissioners (`coManagers`). `ownerId` is canonical; the older `createdByUid` field is a functions-only fallback read ONLY when `ownerId` is absent, never a second owner — when both exist and disagree, the `createdByUid` holder is nobody (`poolOps.isPoolOwnerOrManager`, PLAN-CO-COMMISSIONERS D3; the NFL pick and rename gates follow the same precedence since PLAN-ADMIN-PICK-IMPLICIT-JOIN).
...
**What counts as `FINAL` (2026-09-09, PR #682).** Any ONE of: a non-null `finalizedAt` (the NFL season finalizer, `maybeFinalizeNFLPool`, stamps this and writes NO status — it leaves whatever `status` the pool already had, typically `OPEN` or `LOCKED`, untouched, and other paths such as `backfillPools`, cancel or archive may change it later, so `finalizedAt` is honoured independently of `status`); `isFinal: true`; any `closedVia` other than `ADMIN_CLOSE`; or a raw `status` of `FINAL`, `CANCELED`, `COMPLETED` or `ARCHIVED`, compared **case-insensitively** (the archive path stores lowercase `archived`). `closedVia: 'ADMIN_CLOSE'` wins over all of these and yields `CLOSED`, not `FINAL`. The server's `isTerminalPool` (`functions/src/lib/autoScoreDecisions.ts`) applies the `finalizedAt` / `isFinal` / status parts of this rule before scoring but does NOT read `closedVia` — so a pool carrying only a non-admin `closedVia` is `FINAL` to client readers while still scoreable server-side. Known gap: the functions-side `isFinishedPool` (`lib/poolInclusion.ts`, commissioner aggregate + member-record backfill) does not yet read `finalizedAt` or `FINAL`.
...
### Member Record
The one document per Member per Pool (`pools/{poolId}/members/{uid}`) that is the Roster and payment truth for every Pool type. Carries the Member's identity (`userName`), role, join time, Paid Status, any Member Payment Claim, per-square dues for Squares (`unitsOwned`/`unitsPaid`), and rebuy dues where applicable (`rebuyOwed`/`rebuyPaid`, written in the rebuy transaction). Created when a Member joins (and for the owner when the Pool is created), independent of whether they have played. Deliberately separate from the playable Entry: it exists for pick-less Members and Commissioners, and it does not vary in cardinality or shape by Pool type. Membership is stored in three places that must agree — `pools/{id}.participantIds` (the array every authorization check and roster reader resolves), the Member Record, and the user-side `participations` mirror — and every join path writes all three in one transaction (`stageEnrollment` + `ensureMemberRecord` on the NFL paths; `reconcileMembership` exists in `lib/memberRecord.ts` for the same purpose but has no callers as of 2026-09-10). On the NFL types an ACCEPTED PICK is also a join: a host or a confirmed Super Admin who submits picks without ever calling `joinNFLPool` is enrolled in the same transaction as the Entry, subject to the same seat gates as an explicit join (PLAN-ADMIN-PICK-IMPLICIT-JOIN). Ordinary Members must already be on the Roster to submit.

...
### ✅ Kevin's decision, 2026-08-01: OPTION 2 — narrow the repair jobs

**Chosen after two facts surfaced that the review had not seen:**

1. **This is known, documented debt, not a new discovery.** `SECURITY-CLAIM-SQUARES.md`
   (PR #233) already describes the `guestDeviceKey` finding, recommends the fix
...
**Option 1 could not have completed anyway.** The hash fix is incomplete without
a backfill that hashes and clears every existing raw key — *"the fix is not
complete without it"* — which is a Rule 1 prod-data mutation and a Kevin-only
gate. Securing `claimMySquares` first would have blocked the pilot work on a step
no agent session can finish.

**So the laundering path is cut at the repair jobs instead**, which needs no
production write and lands this week:

- `fixParticipantIds` (`poolOps.ts`) no longer scans `squares[].reservedByUid`.
- `backfillMemberRecords` lets square ownership **enrich** a member, never
  **introduce** one — units apply only to uids another signal established.

Nothing legitimate is lost: `reserveSquare` already adds an authenticated
reserver to `participantIds` at reserve time, so the only uids these blocks ever
ADDED were ones `reserveSquare` had not — which is the guest-claim path.
